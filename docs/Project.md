 ...
 * @dependencies: backend/app.js, public/, pattern1.pdf, package.json, тестовые скрипты
 * @created: 2025-09-13
 */
...
- msr: — справа подставляется максимальный суточный расход воды (м³/сут) и, во второй позиции метки msr:, максимальный секундный расход (л/с)
...
Компоненты:
- Frontend (Vanilla JS + HTML/CSS, `public/`): форма ввода с маппингом полей, валидация, вызов API, загрузка PDF
...
- Форму загрузки PDF шаблона
- Поля ввода данных с возможностью настройки имен меток
- Автоматические расчёты водопотребления
- Генерацию и скачивание заполненного PDF

Каждое поле формы имеет:
- Основное поле ввода данных
- Дополнительное поле для настройки имени метки (справа)
- Placeholder с рекомендуемым именем метки

Блок результатов расчётов включает:
- Отображение расчётных значений (часовой и секундный расход)
- Поля для настройки имен меток для каждого расчётного значения
- Визуальное разделение с помощью стилизованных контейнеров
...
- **Request (JSON)**:
  ...
  - `fieldMappings` (Object, optional) - маппинг полей к именам меток
  - `options` (Object, optional) - параметры отрисовки
- **Response**: application/pdf (attachment)
- **Валидация**: типы, диапазоны, длина строк; HTTP 400 при ошибке

#### Параметры fieldMappings:
- `dailyConsumption` (string) - имя метки для суточного расхода (по умолчанию: "msr_daily")
- `requiredHead` (string) - имя метки для часового расхода (по умолчанию: "mchr")
- `pumpModel` (string) - имя метки для модели насоса (по умолчанию: "n")
- `flowMeter` (string) - имя метки для расходомера (по умолчанию: "flow_meter")
- `projectCode` (string) - имя метки для шифра проекта (по умолчанию: "sh")
- `hourlyConsumption` (string) - имя метки для часового расхода из результатов (по умолчанию: "mchr")
- `secondlyConsumption` (string) - имя метки для секундного расхода из результатов (по умолчанию: "msr_secondly")
...
## Правила подстановки значений в PDF
- **Исходные данные**: подставляются точно как введены пользователем (текст, числа без единиц)
- **Вычисленные данные**: подставляются только в виде цифр без единиц измерения
- **Единицы измерения**: не добавляются автоматически, должны быть указаны в PDF шаблоне
...
### Ветки репозитория
- Основная ветка: `main` (ветка по умолчанию)
- Рабочие ветки: создаются по задаче от `main` с префиксом `feature/`
...
## Конфигурация
- `.env`: `PORT`, `CORS_ORIGIN`, `NUMBER_LOCALE`, `CALCULATION_PRECISION`, `HOURLY_MULTIPLIER`, `SECONDLY_DIVISOR`, `FONT_PATH`
- Шрифты: приоритетно используется `FONT_PATH`; если не задан — `assets/fonts/DejaVuSans.ttf`, иначе fallback на Helvetica
- Примечание: `config/markers.json` признан устаревшим (legacy). Координаты детектируются автоматически через `pdfjs-dist` и кэшируются в манифестах.
