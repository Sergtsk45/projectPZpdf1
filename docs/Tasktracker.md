...
- **Шаги выполнения**:
  ...
  - [x] Зафиксировать изменения и отправить в текущую ветку
  - [x] Проверить историю коммитов на origin
## Задача: Смена главной ветки на main
- **Статус**: Завершена
- **Приоритет**: Высокий
- **Описание**: Переименовать локальную ветку, запушить в origin, удалить старую ветку, назначить main основной.
- **Шаги выполнения**:
  - [x] Переименовать локальную ветку в main
  - [x] Запушить main и назначить upstream
  - [x] Удалить удалённую `feature/hybrid-pdf-architecture`
  - [x] Назначить main веткой по умолчанию в GitHub
- **Зависимости**: [нет]
- **Зависимости**: [нет]
