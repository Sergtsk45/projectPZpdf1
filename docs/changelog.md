...
## [2025-01-13] - Автодетекция пользовательских меток в PDF
### Добавлено
- Поддержка автодетекции пользовательских меток вида `{{name}}` при загрузке шаблона
- Создание полей манифеста с именем, совпадающим с `name`
- Поддержка Unicode символов и пробелов в именах меток `{{имя с пробелами}}`
- Устойчивая обработка маркеров, разбитых на фрагменты в PDF
- Правильное позиционирование значений справа от пользовательских меток

### Изменено
- `backend/src/utils/pdfProcessor.js`: детекция как стандартных маркеров (`msr:`, `n:`, `sh:`, `mchr:`), так и `{{name}}`
- Сборка текста по строкам для корректной обработки разбитых маркеров
- Расширенная регулярная выражение для поддержки кириллицы и пробелов
- Исключение дубликатов пользовательских имен меток в манифесте
- Улучшенное вычисление координат для позиционирования справа от метки

### Исправлено
- Пользовательские метки в PDF автоматически определяются и подставляются без изменения кода
- Поддержка русских имен меток типа `{{суточный расход}}`
- Надежная детекция маркеров даже при разбиении на фрагменты
- Значения корректно размещаются справа от меток `{{name}}` в PDF

## [2025-01-13] - Тестирование с реальным шаблоном pattern1.3.pdf
### Добавлено
- Полное тестирование автодетекции с реальным PDF шаблоном
- Подтверждение работы с меткой `{{xxx}}` в формуле `q = 3,9 {{xxx}} / 24`

### Изменено
- Убраны отладочные логи из production кода

### Исправлено
- Система успешно находит и обрабатывает пользовательские метки в реальных PDF документах
- Значения корректно подставляются справа от меток в формулах
